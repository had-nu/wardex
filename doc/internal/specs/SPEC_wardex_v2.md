...
O risco de release de uma vulnerabilidade é calculado como:

```
adjusted_score    = cvss_base × epss_factor
exposure_factor   = internet_facing_weight × (1 - auth_reduction)
compensated_score = adjusted_score × (1 - compensating_effectiveness)
release_risk      = compensated_score × asset_criticality × exposure_factor
```

Onde:
- `cvss_base` — score CVSS v3.1 da vulnerabilidade (obrigatório)
- `epss_factor` — probabilidade de exploração nos próximos 30 dias segundo EPSS (opcional;
  default: `1.0` quando não fornecido, mantendo conservadorismo)
- `asset_criticality` ∈ [0.0, 1.0] — impacto de negócio se o asset for comprometido,
  declarado pela organização
- `internet_facing_weight` — `1.0` se exposto à internet, `0.6` se interno, `0.3` se air-gapped
- `auth_reduction` ∈ [0.0, 0.4] — redução de exposição quando autenticação é exigida
- `compensating_effectiveness` ∈ [0.0, 0.8] — efetividade combinada dos controles
  compensatórios declarados, clamped em 0.8 para evitar eliminação fictícia do risco
...
- **Testável:** cobertura obrigatória em `pkg/catalog`, `pkg/correlator`, `pkg/scorer`, `pkg/releasegate`, `pkg/accept/signer`, `pkg/accept/verifier`, `pkg/accept/store`, `pkg/accept/audit`
...
├── cmd/
│   └── wardex/
│       └── main.go                # Entry point, subcomandos CLI, orquestração
├── go.mod
├── go.sum
│
├── pkg/
...
│   │   │                          # GatePracticeStatus
│   │   └── acceptance.go          # Acceptance, ContextSnapshot, RevocationRecord, AuditEntry
│   │
│   ├── catalog/
...
│   │   └── gate_test.go
│   │
│   ├── accept/                    # Workflow de aceitação de risco — domínio coeso
│   │   ├── store/
...
│
├── templates/                     # Templates de notificação (accept/)
│   ├── slack-created.tmpl
│   ├── slack-expiring.tmpl
│   ├── slack-expired.tmpl
│   ├── slack-stale.tmpl
...
```
Usage: wardex [command] [flags] <input-file(s)>

# Comando principal — gap analysis + release gate
wardex [flags] <input-file(s)>
...
# Subcomandos de aceitação de risco (pkg/accept/)
...
wardex accept verify-forwarding
  --since       string   Período: ISO 8601 ou relativo (ex: 30d)
  --backend     string   Backend a verificar (default: todos)
...
```bash
# Instalar — um comando instala tudo
git clone https://github.com/had-nu/wardex
cd wardex
go build -o wardex ./cmd/wardex
...
# Workflow de aceitação de risco após BLOCK
wardex accept request \
  --report gate-report.json \
  --cve CVE-2024-1234 \
  --accepted-by "maria.silva@empresa.com" \
  --justification "Componente não reachable em produção até patch v2.2 — confirmado com infra" \
  --expires 30d \
  --ticket "JIRA-4821"
...
wardex accept revoke --id acc-20251014-001 --revoked-by "maria.silva@empresa.com" \
  --reason "Patch v2.2.1 deployado"

# Correr testes com race detector (cobre todos os packages incluindo accept/)
...
| Release Gate Decision      | [BLOCK] BLOCK      |
...
|----------------|------|------|--------------|----------|
| CVE-2024-1234  | 9.1  | 0.84 | **8.7**      | [BLOCK] BLOCK  |
| CVE-2024-5678  | 7.5  | 0.12 | 3.2          | [OK] ALLOW  |
| CVE-2024-9999  | 5.3  | 0.03 | 1.1          | [OK] ALLOW  |
...
| `github.com/aws/aws-sdk-go-v2`      | latest   | Backend CloudWatch em `pkg/accept/forwarder` (build tag: `cloudwatch`) |
| `cloud.google.com/go/logging`       | latest   | Backend GCP Logging em `pkg/accept/forwarder` (build tag: `gcp`)       |

Os backends de cloud são opcionais — compilados apenas com build tags explícitas.
O binário base (`go build ./cmd/wardex`) inclui apenas os backends `http` e `syslog`
sem dependências de cloud SDK.

Sem dependências externas em `pkg/catalog`, `pkg/correlator`, `pkg/scorer`,
`pkg/analyzer`, `pkg/releasegate`, `pkg/accept/signer`, `pkg/accept/verifier`,
`pkg/accept/store`, `pkg/accept/audit`.
...
de compliance crítico), `2` (release bloqueado pelo gate), `3` (adulteração detectada em
aceitações), `4` (inconsistência entre YAML e audit log) permitem que a pipeline tome
decisões autónomas sem parsing de output.

**`pkg/accept/` como domínio coeso dentro do módulo:** todo o workflow de aceitação de
risco vive em `pkg/accept/` e é acessível via subcomandos `wardex accept`. A separação
conceptual entre detecção de risco (packages de análise) e propriedade do risco (accept/)
mantém-se via namespace de packages — não via módulos separados. Isto elimina version skew,
simplifica instalação, e garante que `pkg/accept/signer` e `pkg/releasegate` são sempre
compilados juntos com o mesmo compilador e as mesmas versões de dependências.
